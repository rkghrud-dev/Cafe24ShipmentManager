...
public class Cafe24ApiClient : IMarketplaceApiClient
{
    ...
    private async Task<bool> RefreshAccessTokenAsync(bool allowReauthorize = true)
    {
        ...
        try
        {
            var tokenPath = Environment.ExpandEnvironmentVariables(_config.TokenFilePath ?? "");
            if (File.Exists(tokenPath))
            {
                var fileJson = JObject.Parse(File.ReadAllText(tokenPath));
                var fileRefresh = fileJson["RefreshToken"]?.ToString();
                if (!string.IsNullOrWhiteSpace(fileRefresh) &&
                    !string.Equals(fileRefresh, _config.RefreshToken, StringComparison.Ordinal))
                {
                    _log.Info("토큰 갱신 전: 공유 파일의 최신 refresh_token으로 동기화");
                    _config.RefreshToken = fileRefresh;
                }
            }
        }
        catch (Exception reloadEx)
        {
            _log.Warn($"갱신 전 refresh_token 재로드 실패(무시): {reloadEx.Message}");
        }
